...
; Example .......:
; ===============================================================================================================================
Func _GUICreateUpdate($title, $width, $height, $left = -1, $top = -1, $style = -1, $exStyle = -1, $parent = 0)
	$GUI_Update_in_function = GUICreate($title, $width, $height, $left, $top, $style, $exStyle, $parent)
	GUISetBkColor(0xFFFF99,$GUI_Update_in_function)
	Local $key = String($GUI_Update_in_function)
	Local $obj = $GUI_Update_in_function
	$g_oGuiNotInMini.Add($key, $obj)
	GUISetIcon("csv.ico")
	Return $GUI_Update_in_function
EndFunc   ;==>_GUICreate
		...
		Else
			...
			If $decideupdate = 1 Then
				...
				If String($Key1)= "" And String($Key2)= "" And String($Key3)= "" And String($Key4)= "" And String($Key5)= "" And String($Key6)= ""  Then
					...
					$GUI_Update_in_function = _GUICreateUpdate ("Downloading Autoit" ,360, 100, @DesktopWidth/2-360/2, @DesktopHeight/2-100/2, $WS_BORDER, $WS_EX_CONTROLPARENT)
					$AutoitProgressBar = GUICtrlCreateProgress (5, 5, 350, 20)
					GUISetState(@SW_SHOW, $GUI_Update_in_function)
					...
					While 1
						...
						Else
							...
							Sleep (3000)
							GUISetState ( @SW_HIDE, $GUI_Update_in_function )
							ExitLoop
						EndIf
					WEnd
